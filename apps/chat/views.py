...
def _check_room_member(request, room):
    if not room.member.filter(id=request.user.id).exists():
        return False
    return True
...
class ChatRoomView(APIView):
    ...
    def get(self, request):
        user = request.user
        chatRooms = ChatRoom.objects.filter(member=user.id).annotate(
            unread_count=Count("messages", filter=Q(messages__is_read=False) & ~Q(messages__user=user), distinct=True)
        )
    ...
    def post(self, request):
        ...
        if len(members) == 2:
            user1, user2 = members
            room = ChatRoom.objects.annotate(num_members=Count('member')).filter(type='DM', num_members=2, member__id=user1).filter(member__id=user2).first()
            if room:
        ...
        data = {"members": members, "name": room_name}
        if variant_id:
        ...
        if serializer.is_valid():
            ...
            if variant_id:
                ChatMessage.objects.create(chat=room, user=request.user, product_id=variant.product_id, variant=variant, message_type="product", message="")
            elif product_id:
                product = Products.objects.filter(id=product_id).first()
                if product:
                    ChatMessage.objects.create(chat=room, user=request.user, product=product, message_type="product", message="")
...
class MessagesView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if room and not _check_room_member(self.request, room):
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response({"results": serializer.data, "room_meta": room_meta}, status=status.HTTP_200_OK)
...
class ChatMessageCreateView(APIView):
    ...
    def post(self, request, roomId):
        ...
        if not message and not image and not product_id and not variant_id:
            return Response({"detail": "message, image, product_id ou variant_id requis."}, status=status.HTTP_400_BAD_REQUEST)

        room = ChatRoom.objects.filter(roomId=roomId).first()
        if not room:
            return Response({"detail": "Chat introuvable."}, status=status.HTTP_404_NOT_FOUND)
        if not _check_room_member(request, room):
        ...
        if variant_id:
            ...
            message_type = "product"
            is_intro_product = (message_type == "product" and (message is None or message == "") and image is None)
            if is_intro_product:
                member_ids = list(room.member.values_list("id", flat=True))
                quote_qs = Quote.objects.select_related("shop", "shop__owner", "shop__owner__user", "user").filter(user_id__in=member_ids, shop__owner__user_id__in=member_ids).exclude(shop__owner__user_id=F("user_id"))
                if product and getattr(product, "shop_id", None):
                    quote_qs = quote_qs.filter(shop_id=product.shop_id)
                cutoff = timezone.now() - timedelta(hours=24)
                existing = ChatMessage.objects.filter(chat=room, message_type="product", product=product, variant=variant, timestamp__gte=cutoff).first()
                if existing:
                    terminal_exists = quote_qs.filter(status__in=["rejected", "expired", "converted"]).exists()
        ...
        chat_message = ChatMessage.objects.create(chat=room, user=request.user, message=message or "", image=image, product=product, variant=variant, message_type=message_type)
        ...
        if channel_layer:
            async_to_sync(channel_layer.group_send)(room.roomId, {"type": "chat_message", "message": payload})
        return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class ChatMessageReadView(APIView):
    ...
    def post(self, request, roomId):
        ...
        if message_ids_to_mark:
            channel_layer = get_channel_layer()
            if channel_layer:
                async_to_sync(channel_layer.group_send)(room.roomId, {"type": "chat_message", "message": {"action": "read", "roomId": room.roomId, "user": request.user.id, "message_ids": message_ids_to_mark}})
...
class ChatUserConversationsView(APIView):
    ...
    def get(self, request):
        user = request.user
        rooms = ChatRoom.objects.filter(member=user).annotate(
            unread_count=Count("messages", filter=Q(messages__is_read=False) & ~Q(messages__user=user), distinct=True)
        ...
        for room in rooms:
            ...
            last_msg = ChatMessage.objects.filter(chat=room).order_by('-timestamp').first()
            shop_name = None
            if getattr(other_user, "has_seller_account", False):
                shop = Shops.objects.filter(owner__user=other_user).order_by('date_created').first()
                if shop:
                    shop_name = shop.name
            last_activity = last_msg.timestamp if last_msg else room.last_updated
            response.append({
                "roomId": room.roomId,
                "user": {"id": other_user.id, "first_name": other_user.first_name, "last_name": other_user.last_name, "email": other_user.email, "photo": _user_photo_url(other_user, request), "last_seen": other_user.last_seen.isoformat() if other_user.last_seen else None},
...
class SupportChatStartView(APIView):
    ...
    def post(self, request):
        ...
        def _room_payload(room, agent):
            serializer = ChatRoomSerializer(room, context={"request": request})
            payload = serializer.data
            payload["support_user"] = {"id": agent.id, "first_name": agent.first_name, "last_name": agent.last_name, "email": agent.email, "type_user": getattr(agent, "type_user", ""), "photo": _user_photo_url(agent, request), "last_seen": agent.last_seen.isoformat() if agent.last_seen else None}
            payload["room_type"] = "SUPPORT"
            return payload

        existing_room = ChatRoom.objects.annotate(num_members=Count("member")).filter(type="SUPPORT", num_members=2, member__id=user.id).first()
        if existing_room:
            agent = existing_room.member.exclude(id=user.id).first()
            if agent:
                return Response(_room_payload(existing_room, agent), status=status.HTTP_200_OK)

        candidates = User.objects.filter(is_active=True, type_user='support').exclude(id=user.id).order_by('id')

        def _active_support_chats(agent):
            cutoff = timezone.now() - timedelta(days=7)
            return ChatMessage.objects.filter(chat__type='SUPPORT', chat__member=agent, timestamp__gte=cutoff).values('chat_id').distinct().count()
        ...
        if support_agent is None:
            return Response({"detail": "Aucun agent support disponible pour le moment."}, status=status.HTTP_503_SERVICE_UNAVAILABLE)

        room = ChatRoom.objects.create(type="SUPPORT", name=f"Support: {user.email or user.id}")
        room.member.set([user.id, support_agent.id])
        return Response(_room_payload(room, support_agent), status=status.HTTP_200_OK)
