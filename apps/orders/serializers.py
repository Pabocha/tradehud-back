...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Orders
        fields = [
            'origin_address', 'shipping_method', 'discount', 'coupon_code',
            'order_lines'
        ]
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                country_code = str(order.shipping_country) if order.shipping_country else ''
                shop_ids = list(
                    order.order_lines.values_list('shop_id', flat=True).distinct()
                )
